...
The MSI can take the key as a property, and the wizard field is masked. The key
is also kept out of the verbose install log - the property *and* the command
lines that carry its value are listed in `MsiHiddenProperties`, which is enough
because a log prints every property value it sets.

What that does **not** cover: a key passed on an `msiexec` command line is
visible in the process list while the installer runs, and on machines with
command-line auditing (event 4688) it lands in the security log. If that matters:
