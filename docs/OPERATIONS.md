...
Everything that used to live in the front-page README and was cut when it was
shortened to the essentials. Nothing here is required to get started — the
[README](../README.md) covers that — but this is the place for the full detail:
what each component and panel does, every CLI flag, the common failure modes,
and what to check before you actually turn NTLM off.

## Contents

- [Components](#components)
- [Features](#features)
- [Prerequisites in full](#prerequisites)
...
| **100/101** | DCs *(separate log)* | **Protected Users / authentication policy.** NTLM was rejected because the account is in *Protected Users* (100) or because access control restrictions apply (101) — accounts for which NTLM is already off today. The event names the account, the device it came from and the NT status code (`0xC000006E` = account restriction). Lives in `Microsoft-Windows-Authentication/ProtectedUserFailures-DomainController`. |
| **301** | DCs *(separate log)* | **The early warning.** NTLM succeeded, but would fail once the authentication policy is enforced — the same shape as the October 2026 deadline, from a different direction. Lives in `Microsoft-Windows-Authentication/AuthenticationPolicyFailures-DomainController`. |
| **4010–4012, 4015** | all machines | Blocks from **session security** (`NtlmMinClientSec` / `NtlmMinServerSec`), the DC-generated-secret fallback, and a further outgoing block. These IDs exist in the provider manifest but are **not publicly documented**, so nothing is read by field position — only values that identify themselves are taken, the rest stays in the raw message. |
...
The two Protected Users / authentication policy channels (events 100, 101, 301) are **disabled by default** and exist only on domain controllers. To switch them on: Event Viewer → *Applications and Services Logs → Microsoft → Windows → Authentication* → enable the `ProtectedUserFailures-DomainController` and `AuthenticationPolicyFailures-DomainController` logs, or `wevtutil sl <channel> /e:true`. The agent queries them either way and stays quiet when they are off — nothing else in the tool depends on them.
...
`ntlm-agent.msi` as assets. Between releases, every push to `main` builds both —
in the run's *Artifacts* section, `installer-msi-double-click-this` is the
installer and `agent-exe-only-not-the-installer` is the bare console binary.
